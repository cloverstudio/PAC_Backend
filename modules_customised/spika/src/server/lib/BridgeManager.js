...
var BridgeManager = {
    ...
    init:function(){
        ...
        Observer.subscribe(this, Const.notificationLogin, function(who, obj) {
            
            if(_.isEmpty(Settings.listeners))
                return;
                
            if(_.isFunction(Settings.listeners.OnOpenRoom))
                Settings.listeners.OnOpenRoom(obj);
            
        });

        Observer.subscribe(this, Const.notificationLogout, function(who, obj) {
            
            if(_.isEmpty(Settings.listeners))
                return;
                
            if(_.isFunction(Settings.listeners.OnCloseRoom))
                Settings.listeners.OnCloseRoom(obj);
    ...
    hook: function(method,param,callBack){
        ...
        if(Settings.hooks[method] && _.isFunction(Settings.hooks[method])){
            return Settings.hooks[method](param,callBack);
        }else{
            callBack(null);
        }
