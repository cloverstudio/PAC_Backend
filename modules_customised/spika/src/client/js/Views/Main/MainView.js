...
var MainView = Backbone.View.extend({
    ...
    */
    ...
    adjustSize: function(){
        ...
        var container = Settings.options.defaultContainer;
        
        if(Settings.options.showTitlebar == false){            
            SS('#message-info').css('top','0px');
        }
        
        SS('#message-info').height($(container).height() - SS('#message-info').position().top - SS('#text-message-box-container').height());
        SS('#sidebar').height($(container).height());
        SS('#messaging').height($(container).height());
        SS('#messaging').height($(container).height());
    }
