...
GroupsController.prototype.init = function(app) {
  ...
  router.get("/:groupId", checkAPIKey, (request, response) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(groupId))
      return response
        .status(Const.httpCodeBadParameter)
        .send(Const.errorMessage.groupidIsWrong);
    if (!query)
      return response
        .status(Const.httpCodeBadParameter)
        .send(Const.errorMessage.offsetIsMinus);

    GroupLogic.getDetails(
      groupId,
      query.fields,
      (group, err) => {
        self.successResponse(response, Const.responsecodeSucceed, {
    ...
    async.waterfall(
      [
        ...
        (result, done) => {
          ...
          const userIdList = result.groupDetail.users.slice(
            offset,
            offset + limit
          );
        ...
        [
          ...
          (result, done) => {
            userIds.forEach(userID => {
              userModel.findOne({ _id: userID }, (err, findResult) => {
                if (err || !findResult) return;

                if (!Array.isArray(findResult.groups)) return;
      ...
      async.waterfall(
        ...
        (err, result) => {
          if (err) {
            ...
            this.errorResponse(response, Const.httpCodeServerError);
            return;
          }

          this.successResponse(response, Const.responsecodeSucceed, {
            group: result.groupDetail
          });
        }
      );
...
GroupsController.prototype.validateDuplication = (
  name,
  organizationId,
  callback
) => {
...
GroupsController.prototype.validateUsersPresence = (
  users,
  organizationId,
  callback
) => {
  const userModel = UserModel.get();
  const conditions = {
    $and: [{ organizationId: organizationId }, { _id: { $in: users } }]
  };
