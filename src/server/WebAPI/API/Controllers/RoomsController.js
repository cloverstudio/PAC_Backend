...
RoomsController.prototype.init = function (app) {
  ...
  router.get("/:roomId", checkAPIKey, (request, response) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(roomId))
      return response
        .status(Const.httpCodeBadParameter)
        .send(Const.errorMessage.roomidIsWrong);
    if (!query)
      return response
        .status(Const.httpCodeBadParameter)
        .send(Const.errorMessage.offsetIsMinus);

    RoomLogic.getDetails(
      roomId,
      query.fields,
      (room, err) => {
        self.successResponse(response, Const.responsecodeSucceed, {
    ...
    async.waterfall(
      [
        ...
        (result, done) => {
          ...
          const userIdList = result.roomDetail.users.slice(
            offset,
            offset + limit
          );
      ...
      [
        ...
        (result, done) => {
          ...
          if (result.roomDetail.owner != request.user._id)
            return response
              .status(Const.httpCodeForbidden)
              .send("No Permission");
        ...
        (result, done) => {
          ...
          roomModel.update(
            ...
            (err, updated) => {
              if (!err) result.roomDetail.users = result.newUsers;
...
RoomsController.prototype.validateUsersPresence = (
  users,
  organizationId,
  callback
) => {
