    ...
    router.post('', function (request, response) {
        ...
        async.waterfall([
            ...
            function (result, done) {
                ...
                var newVoipPushToken = request.body.voipPushToken;
                ...
                if (UUIDSaved.length > 0) {
                    ...
                    uuidAry = _.map(uuidAry, (uuidObj) => {
                        ...
                        if (uuidObj.UUID == UUID) {
                            ...
                            if (!uuidObj.pushTokens)
                                uuidObj.pushTokens = [];

                            if (!_.isEmpty(newPushToken) && uuidObj.pushTokens.indexOf(newPushToken) == -1)
                                uuidObj.pushTokens.push(newPushToken);

                            if (!_.isEmpty(newVoipPushToken) && uuidObj.pushTokens.indexOf(newVoipPushToken) == -1)
                                uuidObj.pushTokens.push(newVoipPushToken);
                        }
                    ...
                    if (!_.isEmpty(newPushToken))
                        UUIDObj.pushTokens.push(newPushToken);

                    if (!_.isEmpty(newVoipPushToken))
                        UUIDObj.pushTokens.push(newVoipPushToken);
