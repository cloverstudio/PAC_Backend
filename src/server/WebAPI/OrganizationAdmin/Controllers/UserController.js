        ...
        async.waterfall([
            ...
            function (result, done) {
                ...
                self.getGroups(baseUser, GroupModel, Const.groupType.department, function (err, departmentData) {
                    ...
                    result.departmentList = departmentData;
                    result.allGroups = result.groupList.concat(result.departmentList);

                    result.allGroupIds = result.allGroups.map((group) => {
            ...
            function (result, done) {
                ...
                const joinedGroups = result.originalData.groups;

                // remove all groups of the admin
                const groupsOfOtherAdmin = joinedGroups.filter((groupId) => {

                    return result.allGroupIds.indexOf(groupId) == -1;
                ...
                var updateParams = {
                    ...
                    groups: newGroups.concat(groupsOfOtherAdmin)
                };
                ...
                if (templateParams.file) {
                    ...
                    model.update({ _id: _id }, updateParams, function (err, updateResult) {
                        ...
                        var modelGroup = GroupModel.get();
                        const allGroups = result.groupList.concat(result.departmentList);
                        ...
                        const allGroupIds = allGroups.map((group) => {
